import type { OverlayState, AttentionRegion, AttentionType } from '../../shared/types';
...
function colorFor(type: AttentionType, theme: OverlayState['colorTheme']): [number, number, number] {
  switch (type) {
...
export function drawAttentionHighlights(
  ...
  if (!attention.enabled || !attention.regions || attention.regions.length === 0) return;
...
function drawRegionHighlight(
  ...
  ctx.beginPath();
  ctx.moveTo(sx, sy + bl); ctx.lineTo(sx, sy); ctx.lineTo(sx + bl, sy);
  ctx.moveTo(sx + sw - bl, sy); ctx.lineTo(sx + sw, sy); ctx.lineTo(sx + sw, sy + bl);
  ctx.moveTo(sx + sw, sy + sh - bl); ctx.lineTo(sx + sw, sy + sh); ctx.lineTo(sx + sw - bl, sy + sh);
  ctx.moveTo(sx + bl, sy + sh); ctx.lineTo(sx, sy + sh); ctx.lineTo(sx, sy + sh - bl);
  ctx.stroke();
