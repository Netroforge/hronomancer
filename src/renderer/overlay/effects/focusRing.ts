...
function strokePerimeter(
  ...
  ctx.beginPath();
  ctx.moveTo(x0, y0);
  let seg = Math.min(remaining, hSpan); ctx.lineTo(x0 + seg, y0); remaining -= seg;
  if (remaining > 0) { seg = Math.min(remaining, vSpan); ctx.lineTo(x1, y0 + seg); remaining -= seg; }
  if (remaining > 0) { seg = Math.min(remaining, hSpan); ctx.lineTo(x1 - seg, y1); remaining -= seg; }
  if (remaining > 0) { seg = Math.min(remaining, vSpan); ctx.lineTo(x0, y1 - seg); }
  ctx.stroke();
}
