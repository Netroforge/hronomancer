...
function getCpuColor(pct: number, r: number, g: number, b: number): string {
  if (pct > 80) return '#ff2a6d';
  if (pct > 50) return '#faff00';
  return `rgb(${r},${g},${b})`;
}

function getRamColor(pct: number, r: number, g: number, b: number): string {
  if (pct > 85) return '#ff2a6d';
  if (pct > 60) return '#faff00';
  return `rgb(${r},${g},${b})`;
}
