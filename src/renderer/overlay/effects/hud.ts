...
let vignetteCache: { grad: CanvasGradient; W: number; H: number; intensity: number } | null = null;
...
function loadColor(base: [number, number, number], load: number): [number, number, number] {
  const amber: [number, number, number] = [255, 176, 32];
  const red: [number, number, number] = [255, 48, 32];
  if (load <= 0) return base;
  if (load < 0.6) {
    const t = load / 0.6;
...
export function drawEdgeGlow(
  ...
  ctx.moveTo(W, H - cornerLen); ctx.lineTo(W, H); ctx.lineTo(W - cornerLen, H);
  ctx.moveTo(cornerLen, H); ctx.lineTo(0, H); ctx.lineTo(0, H - cornerLen);
  ctx.stroke();
  ctx.restore();
  ...
  if (edgeFlash) {
    ...
    if (edgeFlash.top > 0.01) { ctx.globalAlpha = edgeFlash.top * 0.85; ctx.fillRect(0, 0, W, thick); }
    if (edgeFlash.bottom > 0.01) { ctx.globalAlpha = edgeFlash.bottom * 0.85; ctx.fillRect(0, H - thick, W, thick); }
    if (edgeFlash.left > 0.01) { ctx.globalAlpha = edgeFlash.left * 0.85; ctx.fillRect(0, 0, thick, H); }
    if (edgeFlash.right > 0.01) { ctx.globalAlpha = edgeFlash.right * 0.85; ctx.fillRect(W - thick, 0, thick, H); }
    ctx.restore();
  }
}
...
export function drawSysTag(
  ctx: CanvasRenderingContext2D,
  W: number,
  H: number,
  state: OverlayState,
): void {
  if (!state.showSysTag) return;
  const { intensity, colorTheme } = state;
  const [r, g, b] = hexToRgb(themeColor(colorTheme, 'primary'));
...
export function drawActivityBar(
  ctx: CanvasRenderingContext2D,
  state: OverlayState,
): void {
  ...
  // Fill
  let color = '#00ff41';
  if (activityLevel > 0.6) color = '#faff00';
  if (activityLevel > 0.8) color = '#ff2a6d';
  ctx.fillStyle = color;
