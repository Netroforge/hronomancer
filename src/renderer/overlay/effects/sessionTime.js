...
function rampColor(base, ratio) {
    const amber = [255, 176, 32];
    const red = [255, 72, 48];
    if (ratio <= 0)
        return base;
    if (ratio < 1)
        return [lerp(base[0], amber[0], ratio), lerp(base[1], amber[1], ratio), lerp(base[2], amber[2], ratio)];
...
export function drawSessionTime(ctx, W, H, state, time) {
    if (!state.showSessionTime)
        return;
    ...
    const [r, g, b] = rampColor(base, ratio).map(Math.round);
    const PW = 118;
    const PH = 46;
    ctx.save();
