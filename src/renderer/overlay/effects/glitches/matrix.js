...
const GREEN = '#00ff41';
const HEAD = '#c8ffd0';
function stream(x, y, bottom, len, life) {
...
export const matrixGlitch = {
    ...
    spawn(c) {
        ...
        if (c.trigger === 'key') {
            if (on(c, 'cascade'))
                out.push(stream(c.x + rand(-30, 30), c.y, c.H, [4, 8], [700, 1200]));
            return out;
        }
        if (c.trigger === 'click') {
            if (on(c, 'cascade')) {
                ...
                for (let i = 0; i < n; i++)
                    out.push(stream(c.x + rand(-40, 40), c.y + rand(-20, 20), c.H, [4, 10], [600, 1100]));
            }
            return out;
        }
