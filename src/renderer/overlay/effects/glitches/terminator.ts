...
export const terminatorGlitch: GlitchTheme = {
  id: 'terminator',
  ...
  spawn(c: SpawnContext): Glitch[] {
    ...
    if (c.trigger === 'key') {
      if (on(c, 'codeDump')) out.push(makeLabel({ x: c.x + rand(-10, 10), y: c.y - 12, text: pick(CODE), color: HOT, size: 12, life: rand(600, 1000), weight: 400 }));
      return out;
    }

    if (c.trigger === 'click') {
      if (on(c, 'reticle')) out.push(makeReticle({ x: c.x, y: c.y, size: rand(20, 40), color: RED, life: rand(700, 1200) }));
      if (on(c, 'callouts')) out.push(makeLabel({ x: c.x + 24, y: c.y - 20, text: pick(CALLOUTS), color: HOT, size: 11, life: rand(600, 1000) }));
      return out;
    }
