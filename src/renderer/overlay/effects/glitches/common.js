...
export const on = (c, key) => c.params[key]?.enabled ?? true;
export const amt = (c, key) => c.params[key]?.intensity ?? 1;
// ─── Tiny RNG / util helpers ────────────────────────────────────
export const rand = (a, b) => a + Math.random() * (b - a);
export const randInt = (a, b) => Math.floor(rand(a, b + 1));
export const pick = (arr) => arr[Math.floor(Math.random() * arr.length)];
export const pickChar = (s) => s.charAt(Math.floor(Math.random() * s.length));
...
export function makeBar(o) {
    ...
    return {
        ...
        draw(ctx) {
            ...
            if (k > 0.5)
                ctx.fillRect(x, o.y + o.h + 2, o.w * 0.6, 1);
            ctx.restore();
        },
    };
}
...
export function makeLabel(o) {
    ...
    return {
        ...
        draw(ctx) {
            if (Math.random() < 0.12)
                return; // flicker: occasionally drop a frame
            const k = life.remaining();
            ctx.save();
            ctx.globalAlpha = k;
