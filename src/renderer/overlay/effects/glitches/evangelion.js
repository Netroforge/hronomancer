...
export const evangelionGlitch = {
    id: 'evangelion',
    signature: 'NERV alert — pulsing AT-field hexagons, hazard bars and 警告 warnings.',
    spawn(c) {
        const out = [];
        if (c.trigger === 'key') {
            if (on(c, 'warningLabels'))
                out.push(makeLabel({ x: c.x + rand(-10, 10), y: c.y - 14, text: pick(WARNINGS), color: pick([RED, ORANGE, GREEN]), size: 14, life: rand(600, 1100) }));
            return out;
        }
        if (c.trigger === 'click') {
            if (on(c, 'hexWarnings'))
                out.push(makeHex({ x: c.x, y: c.y, r: rand(24, 48), color: pick([RED, ORANGE]), life: rand(700, 1200), rot: rand(0, Math.PI) }));
            if (on(c, 'warningLabels'))
                out.push(makeLabel({ x: c.x + 20, y: c.y - 24, text: pick(['警告', 'パターン青', '使徒']), color: RED, size: 13, life: rand(600, 1000) }));
            return out;
        }
