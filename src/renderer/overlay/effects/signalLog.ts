...
function iconFor(type: SignalEvent['type']): string {
  switch (type) {
...
function colorFor(type: SignalEvent['type'], theme: OverlayState['colorTheme']): [number, number, number] {
  switch (type) {
...
export function drawSignalLog(
  ...
  events: SignalEvent[],
  now: number,
  recap: boolean, // just returned from away → reframe as a recap
  time: number,
): void {
  if (!state.showSignalLog || events.length === 0) return;
  ...
  const accent: [number, number, number] = recap
    ? hexToRgb(themeColor(state.colorTheme, 'accent'))
    : [pr, pg, pb];
  const { x, y } = drawPanel(ctx, panelX, panelY, PW, PH, accent.map((c) => Math.round(c * pulse)) as [number, number, number]);
