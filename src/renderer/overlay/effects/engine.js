import { spawnGlitches } from './glitches';
...
import { drawVignette, drawEdgeGlow, drawStatusHud, drawSysTag, drawActivityBar } from './hud';
...
import { drawTaskComplete, DONE_PULSE_MS } from './taskComplete';
import { drawNotificationRadar, RADAR_PING_MS } from './notificationRadar';
...
export class CyberEngine {
    ...
    edgeFlash = { top: 0, right: 0, bottom: 0, left: 0 };
    ...
    flashedIds = new Set();
    radarSeenIds = new Set();
    completedSeenIds = new Set();
    ...
    emitGlitches(trigger, x, y, amount) {
    ...
    flashEdgeFor(cx, cy) {
        ...
        if (min === top)
            this.edgeFlash.top = 1;
        else if (min === bottom)
            this.edgeFlash.bottom = 1;
        else if (min === left)
            this.edgeFlash.left = 1;
        else
            this.edgeFlash.right = 1;
    }
    ...
    processAttentionEvents() {
        ...
        if (!att?.enabled || !att.regions) {
            this.donePulses.length = 0;
            this.radarPings.length = 0;
            return;
        }
        const live = new Set();
    ...
    isActive() {
        ...
        if (!s.bootComplete)
            return true;
        if (s.activityLevel > 0.02)
            return true;
        if (this.glitches.length > 0)
            return true;
        if (this.trail.length > 0)
            return true;
        if (s.notificationFlash > 0)
            return true;
        if (s.showAudioViz && this.audioActive)
            return true;
        if (s.attention?.regions?.length > 0)
            return true;
        if (this.donePulses.length > 0)
            return true; // task-done pulse animating
        if (this.radarPings.length > 0)
            return true; // notification radar animating
        if (this.focusStrength > 0.02)
            return true; // focus spotlight engaged / easing out
        if (Date.now() < this.recapUntil)
            return true; // "while you were away" recap breathing
        if (s.presence && s.presence.breakLevel > 0)
            return true; // keep the break nudge breathing
        if (Math.abs(this.presenceAlpha - this.presenceTarget()) > 0.01)
            return true; // mid dim/undim
        if (this.systemLoad() > 0.7)
            return true; // keep the stress pulse smooth
        return false;
    }
    render(time) {
        const ctx = this.ctx;
        ...
        // Notification flash
        if (this.state.showNotificationFlash)
            drawNotificationFlash(ctx, this.W, this.H, this.state);
        ...
        if (this.state.cinemaMode)
            suppressVideoRegions(ctx, this.W, this.H, this.state);
        ...
        if (this.state.showTaskComplete)
            drawTaskComplete(ctx, this.W, this.H, this.donePulses, time, this.state);
        // Edge glow — hue/pulse encode system load; flashes toward notifications.
        if (this.state.showEdgeGlow)
            drawEdgeGlow(ctx, this.W, this.H, this.state, time, load, this.edgeFlash);
        // Focus ring — perimeter progress (pomodoro time left, else the hour).
        drawFocusRing(ctx, this.W, this.H, this.state);
        // Vignette
        if (this.state.showVignette)
            drawVignette(ctx, this.W, this.H, this.state);
        // Activity bar
        if (this.state.showActivityBar)
            drawActivityBar(ctx, this.state);
        // Clock
